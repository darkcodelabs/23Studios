...
function buildSceneLuaFromFeatures(scene, featureSet, recipeBody) {
  ...
  return [
    ...
    `    audio_manager.play_music("sounds/music/${id}", { fade_ms = 250 })`,
    `  end`,
    `end`,
    '',
    ...
    `    -- Auto-advance: A press transitions to the first declared exit. Scenes`,
    `    -- with bespoke A handlers via mechanic_kit recipes override above this.`,
    `    local first_exit = next(exits)`,
    `    if first_exit then self:transition_to(first_exit) end`,
    `  end`,
    `end`,
    '',
    ...
    `    return`,
    `  end`,
    `  -- Playdate import is static-string only; main.lua imports every`,
    `  -- scene module up-front and binds globals via _G. We resolve the`,
    `  -- target by global lookup, NOT dynamic import.`,
    `  local next_scene = _G[target.to] or _G["Scene_" .. target.to]`,
    `  if not next_scene then`,
    `    print("scene_${ident}: failed to load scene " .. tostring(target.to))`,
    `    return`,
    `  end`,
    `  if scene_manager and scene_manager.replace then`,
